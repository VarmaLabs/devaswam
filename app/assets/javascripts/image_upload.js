...
// Remove a image uploader
function remove_fields(link) {
    $(link).prev("input[type=hidden]").val("1");
    $(link).closest("div.add-images-path").hide();
}
  ...
  if(width == undefined){
    ...
    if(status == "false") {
        ...
        if(navigator.userAgent.match(/msie/i) && !navigator.userAgent.match(/msie 10/i)) {
            if($('.div-sg-image-preview').length > 0) {
                $('.div-sg-image-preview').css('border', '1px solid lightgray').html(filename);
            }
            else {
                $(input).parents('.add-images-path').find('.jquery-sg-image-container').
